...
| Slash-command driven session UX / slash command 驱动的会话体验 | We already support `/help`, `/status`, `/tools`, `/files`, `/cost`, `/doctor`, `/version`, `/clear`, `/resume`, `/rename`, `/model`, `/provider`, `/base-url`, `/exit` (`/quit`). / 当前已经支持一组核心 slash commands。 | [cli/chat/command](src/main/java/com/example/codingagent/cli/chat/command), [ChatSlashCommandDispatcher.java](src/main/java/com/example/codingagent/cli/chat/command/ChatSlashCommandDispatcher.java) |
